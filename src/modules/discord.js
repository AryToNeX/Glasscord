...
module.exports = class Discord extends Module{
    ...
    constructor(main){
		...
		this.main.win.webContents.on('dom-ready', () => {
            this.executeMethod(this._getWebpackModules);
        });

        this.main.win.webContents.on('did-finish-load', async () => {
            if (!this.config.titlebar || this.config.titlebar === 'native') return;
            const success = await this.executeMethod(this._changeTitlebar, this.config.titlebar);
	...
	async update(cssProp, value){
		if (cssProp === '--glasscord-titlebar') {
            ...
            if (os === this.currentTitlebar) this.executeMethod(this._popModal);
            else this.executeMethod(this._showConfirmationModal, 'Restart Needed', 'The theme you are using makes use of a custom titlebar that requires a restart to take effect. Would you like to restart now?')
        }
    }
